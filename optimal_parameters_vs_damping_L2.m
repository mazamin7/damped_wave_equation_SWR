...
nu_vals    = logspace(-5, 0, Nn);   % Vary Nu from 1e-5 to 1
...
% Determine axis limits dynamically based on data range
p_data = p_opt(1,:);
q_data = q_opt(1,:);
...
plot(x(1), y(1), 'ko', 'MarkerSize', 10, 'MarkerFaceColor', 'k'); % Start
text(x(1), y(1), '  Low \nu', 'FontSize', 14, 'FontWeight', 'bold');

plot(x(end), y(end), 'ks', 'MarkerSize', 10, 'MarkerFaceColor', 'k'); % End
...
semilogx(nu_vals, rho_opt(1,:), 'b-', 'LineWidth', LW_BOLD);
