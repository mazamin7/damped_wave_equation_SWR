...
for ii = 1:numel(N_list)
    ...
    v0_fun = @(x,y) exp(-sqrt((x-Lx/3).^2 + (y-Ly/3).^2));

    % reference and initial guess
    fprintf('[N=%d] building reference...\n',N);
    tic;
    u_ref  = run_fdtd_2D(u0_fun, v0_fun, Lx, Ly, T, c, dh, dt, gamma, nu);
    tref = toc; %#ok<NASGU>
    ...
    tic;
    [~, final_res(ii), ~] = run_swr_2D(u0_fun, v0_fun, N, a_val, M, Ly, T, c, dh, dt, gamma, nu, p, q, k, u_init, u_ref);
    time_robin(ii) = toc;
