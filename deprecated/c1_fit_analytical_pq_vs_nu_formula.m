...
[Coeffs_p_quad, ~] = polyfit(nu_vals.^2, p_real, 1);
p_approx_quad = polyval(Coeffs_p_quad, nu_vals.^2);

% For comparison, let's look at the Intercept and Slope
p_intercept = Coeffs_p_quad(2);
p_curvature = Coeffs_p_quad(1);

fprintf('Fitted Models:\n');
fprintf('  q(nu) = %.4f * nu + %.4e\n', Coeffs_q(1), Coeffs_q(2));
fprintf('  p(nu) = %.4f + (%.4f) * nu^2\n', p_intercept, p_curvature);
...
plot(nu_vals, p_approx_quad, 'r-', 'LineWidth', 2, 'DisplayName', 'Quadratic Fit (A\nu^2 + C)');
xlabel('\nu'); ylabel('p');
title('p vs \nu (Quadratic)');
legend('Location','best');
axis square;
...
plot(nu_vals.^2, p_real, 'ko', 'MarkerFaceColor','k', 'DisplayName', 'Data');
plot(nu_vals.^2, p_approx_quad, 'r-', 'LineWidth', 2, 'DisplayName', 'Fit');
xlabel('\nu^2'); ylabel('p');
